...
import sys, json, decimal, warnings, io
...
class Backend:
    ...
    def _param_placeholder_function(self, paramstyle):
        ...
        pyformat = dbi_placeholder_format

        return locals()[paramstyle]
    
    def __init__(self, dpi_module, connection):
        self.connection = connection
        
        self.param_placeholder = self._param_placeholder_function(
            dpi_module.paramstyle)
...
class sql:
    ...
    def print(self, *args, **kw):
        for a in args:
            if isinstance(a, literal):
                ...
                print(self.ds.param_placeholder(len(self._parameters)),
                      file=self._sql, **kw)
            else:
                print(a, file=self._sql, end="", **kw)
                
                #raise TypeError("You can only print strings and Parameters "
                #                "to an sql object.")
    ...
    def __call__(self, *args):
        ...
        return ( self._sql.getvalue(),
                 tuple(map(lambda literal: literal.value, self.parameters)), )

    def debug(self, *args):
        """
        Use the __debug__() method instead of __sql__() to construct
        a logable version of the SQL command or query in *args.
        """
        for arg in args:
            if hasattr(arg, "__debug__"):
                arg.__debug__(self)
            else:
                self.print(arg)
...
class _Part:
    ...
    def __debug__(self, runner):
        sql, params = self.__sql__(runner)
        return sql
    
    def __str__(self):
        return sql.debug(Backend())(self)
...
class identifyer(_Part):
    ...
    def __sql__(self, runner):
        if self._quotes:
            return runner.ds.quote_identifyer(self._name)
        else:
            return self._name
...
class direct_literal(literal):
    ...
    """
    def __sql__(self, runner):
        runnert.print(self._value)
        
    __debug__ = __sql__

class typed_literal(literal):
    type = None

    def __init__(self, value):
        if type(value) != self.type:
            msg = "%s takes an %s-instance as argument, not a %s" % (
...
class typed_direct_literal(typed_literal, direct_literal):
    pass
    
    
class integer_literal(typed_direct_literal):
    type = int
        
class float_literal(typed_direct_literal):
    type = float
...
class string_literal(literal):
    ...
    def __debug__(self, runner):
        return runner.quote_string(self.value)

class bytes_literal(literal):
    type = bytes
    
    def __debug__(self, runner):
        return "<%i bytes>" % len(self._value)

class bool_literal(typed_literal):
    type = bool
...
class json_literal(string_literal):
    def __init__(self, data):
        string_literal.__init__(self, json.dumps(data))
...
class relation(_Part): 
    ...
    def __sql__(self, runner):
        if self._schema is not None:
            runner(self._schema, sep="")
            runner.print(".", sep="")
            runner.print(self._name)
        else:
            runner.print(self._name)
